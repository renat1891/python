...
class TicTacToe:
    ...
    def make_click_handler(self, i, j):
        def handler():
            self.click(self.buttons[i][j])
        return handler

    def get_turn_text(self):
        return f"Хід: {'Ти' if self.player == 'X' else 'Комп’ютер'}"

    def click(self, btn):
        if btn["text"] == "" and self.player == "X":
            btn["text"] = "X"
            if self.check_win():
                self.lbl.config(text="Ти переміг!")
                self.disable_all()
            elif not any(b["text"] == "" for row in self.buttons for b in row):
                self.lbl.config(text="Нічия!")
            else:
                self.player = "O"
                self.lbl.config(text="Хід комп’ютера...")
                self.win.after(500, self.bot_move)
    ...
    def bot_move(self):
        ...
        if empty:
            ...
            if self.check_win():
                self.lbl.config(text="Комп’ютер переміг!")
                self.disable_all()
            elif not any(b["text"] == "" for row in self.buttons for b in row):
                self.lbl.config(text="Нічия!")
            else:

...
# def change():
#     course["text"] = f"Курс {var.get()}"

# Label(window, text="Виберіть валюту", font=("Arial", 12)).place(x=60, y=10)

# var = IntVar()
# var.set(currency["usd"])
# Radiobutton(window, text="долар", variable=var, value=currency["usd"], command=change).place(x=40, y=40)
# Radiobutton(window, text="євро", variable=var, value=currency["eur"], command=change).place(x=120, y=40)

# course = Label(window, text=currency["usd"])
# course.place(x=40, y=70)
